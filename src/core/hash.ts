 ...
 *
 * Uebernommen aus dem Nachbarprojekt InfiniteSettler (src/sim/hash.ts) und
 * um hash3i erweitert, weil unsere Chunks zusaetzlich ein Salt brauchen:
 * Gelaende, Zahlen und Haefen ziehen aus demselben Chunk drei voneinander
 * unabhaengige Zufallsstroeme.
 */
...
/** Wie hash2i, aber mit einem dritten Wert (Salt / Ebene). Liefert uint32. */
export function hash3i(seed: number, x: number, y: number, z: number): number {
  let h = hash2i(seed, x, y) | 0;
...
/** FNV-1a ueber einen String. Fuer Zustands-Hashes in Determinismus-Tests. */
